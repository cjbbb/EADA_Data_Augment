"""
    This is a script used to process the conll2003 dataset

    Generate active_entities:
    Input: conll2003/eng.train,
    Output: conll2003/active_entities,  conll2003/train.txt

    Generate active_packages:
    Input: conll2003/active_entities. conll2003/train.txt
    Output: conll2003/active_packages,
"""
...
def run():
    with open('conll2003/eng.train', 'r', encoding='utf-8') as file:
        with open('conll2003/train.txt', 'w', encoding='utf-8') as fw:
            words = []
            readSlots = []
            while (1):
                line = file.readline();
                if not line:  # 终止
                    break;
                if line != '\n':
                    lineList = line[0:-1].split()  # 用' '分开
                    if (lineList[0] != ',') & (lineList[0] != '\t') & (lineList[0] != '.') & (
                            lineList[0] != '-DOCSTART-'):  # 不要标点符号
                        words.append(lineList[0])  # 第一个为单词
                        readSlots.append(lineList[-1])
                else:
                    sentLine = ' '.join(words)
                    for i in range(len(readSlots)):
                        if (i == 0) & (readSlots[i][0] == 'I'):
                            readSlots[i] = 'B-' + readSlots[i][2:]
                        else:
                            if (readSlots[i - 1][0] == 'O') & (readSlots[i][0] == 'I'):
                                readSlots[i] = 'B-' + readSlots[i][2:]
                            if (readSlots[i][0] == 'I') & (readSlots[i - 1] == 'I'):
                                if (readSlots[i][2:] != readSlots[i - 1][2:]):  # 不一致
                                    readSlots[i] = 'B-' + readSlots[i][2:]

                    slotsLine = ' '.join(readSlots)
                    Lens = len(readSlots)
                    words.clear()  # 新句子重置
                    readSlots.clear()
...
def genEntity():
    for key in entityMap:
        if len(key) >= 1:  # 可能出现’‘
            with open('conll2003/active_entities/' + key, 'w', encoding='utf-8') as fw:
                for value in entityMap[key]:
                    fw.write(value + '\n')


'''
    This part is responsible for generating active_packages
    Input: conll2003/active_entities, conll2003/train.txt
    Output: conll2003/active_packages.
'''

entities = os.listdir('conll2003/active_entities')
entitylist = []
tempCount = 0
...
def generate_packages():
    ...
    with open('conll2003/train.txt', 'r', encoding='utf-8') as fin:
        for line in fin:
            count += 1
            query = eval(line)
    ...
    for kind in train:
        with open('conll2003/active_packages/%s' % kind + '.json', 'w', encoding='utf-8') as fout:
...
if __name__ == "__main__":
    run()
    print('finish file: conll2003/train.txt')
    genEntity()
    print('Generated Entity with ' + str(len(entityMap) - 1) + ' entity')
    print('number' + ' ' + str(len(sentenceList)))
    generate_packages()
    print('finish packages generation, location: conll2003/active_packages')

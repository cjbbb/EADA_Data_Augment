...
def hierarchy(data, parent=None, index=0):
    ...
    if node.data['type'] == 'root':
        ...
        if node.data['isSlot']:
            if 'entity' not in data:
                raise_error(
                    'Key "entity" not found nearby "...%s...", slot content node must contains key "entity".' % str(
                        data)[:64])
    ...
    if node.data['type'] in ('root', 'pickone', 'intent'):
        ...
        node.weights = array(node.weights, dtype=float32)
        if node.data['type'] == 'root':
            node.weights /= node.weights.sum()
            #print(node.weights.sum())
            node.weights *= total_count
            print(node.weights)
            for i in range(len(node.weights)):
                node.weights[i] = int(node.weights[i])
    return True, node, stat

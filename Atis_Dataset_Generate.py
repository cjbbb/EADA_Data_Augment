"""
    This script is used to generate entites and packages for ATIS dataset.
    Entity part:
    Input: atis/train.txt
    Output: atis/active_entities

    Packages part:
    Input: atis/train.txt
    Output: atis/active_packages
"""
...
def generate_packages_aits():
    train = {}
    count = 0
    with open('atis/train.txt', 'r', encoding='utf-8') as fin:
        for line in fin:
            count += 1
            query = eval(line)
    ...
    for kind in train:
        with open('atis/active_packages/%s' % kind + '.json', 'w', encoding='utf-8') as fout:
...
"""
    Generate entities for Atis dataset.
"""


def generate_entities_atis():
    entities = {}
    ...
    for kind in entities:
        with open('atis/active_entities/%s' % kind, 'w', encoding='utf-8') as fout:
            for item in entities[kind]:
                fout.write('%s\n' % item)


if __name__ == "__main__":
    generate_entities_atis();
    print('finish file: atis/active_entities')
    generate_packages_aits()
    print('finish file: atis/active_packages')

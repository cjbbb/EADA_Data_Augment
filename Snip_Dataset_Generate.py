'''
    This is used to generate train.txt data for snips dataset.

    Entity part:
    Input: snips/train/label, snips/train/seq.in, snips/train/seq.out
    Output: snips/train.txt, snips/active_entities

    Packages part:
    Input: snips/train.txt, snips/active_entities,
    Output: snips/active_packages.
'''
import json
import os

'''
    This part is responsible for packages
'''
entities = os.listdir('snips/active_entities')
entitylist = []
tempCount = 0
...
def gen_packages():
    ...
    with open('snips/train.txt', 'r', encoding='utf-8') as fin:
        for line in fin:
            count += 1
            query = eval(line)
    ...
    for kind in train:
        with open('snips/active_packages/%s' % kind + '.json', 'w', encoding='utf-8') as fout:
...
'''
    This part is responsible for entity
'''
ansList = []
    ...
    with open('snips/train/label', 'r', encoding='utf-8') as frLabel:
        with open('snips/train/seq.in', 'r', encoding='utf-8') as frSent:
            with open('snips/train/seq.out', 'r', encoding='utf-8') as frSlots:
                with open('snips/train.txt', 'w', encoding='utf-8') as fw:
                    while (1):
...
def genEntity():
    for key in entityMap:
        if len(key) >= 1:  # 可能出现’‘
            with open('snips/active_entities/' + key, 'w', encoding='utf-8') as fw:
...
if __name__ == "__main__":
    run()
    print('finish: snips/ train.txt')
    genEntity()
    print('Entity generation finished, with ' + str(len(entityMap) - 1) + ' entities')
    gen_packages()
    print('finish: snips/active_pacakges')
